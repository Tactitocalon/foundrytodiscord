...
export function messageParserPF1(msg) {
    ...
    if (game.modules.get('monks-tokenbar')?.active && generic.tokenBar_isTokenBarCard(msg.content)) {
        ...
        */
        ...
        // Extract flavor from content
        let extractedFlavor = extractFlavorText(msg.content)
        if (extractedFlavor != null && extractedFlavor.length > 0) {
...
function extractFlavorText(str) {
    let regex = /<span class="flavor-text">([\s\S]*?)<\/span>/g;
    let result = regex.exec(str);
    if (result == null) {
        return "";
    }
    return removeHtmlTags(result[1]);
}
...
function PF2e_createRollEmbed(message) {
    ...
    let extractedFlavor = extractFlavorText(message.content).replace(/^\s+|\s+$/g, '');
    if (extractedFlavor && extractedFlavor.length > 0) {
        title = (message.alias.replace(/^\s+|\s+$/g, '') + ": " + extractedFlavor);
        if (desc !== "") {
            desc = desc + "\n";
        }
        for (let i = 0; i < message.rolls.length; i++) {
            let resultStr = message.rolls[i].result.replace("+  -", "-");
            let formulaStr = message.rolls[i].formula.replace("+  -", "-");
            desc = desc + "**:game_die:Result: **" 
                + resultStr + " = " + "__**" + message.rolls[i].total + "**__\n*(" + formulaStr + ")*";
            desc = desc + "\n";
        }
    }
    else {
